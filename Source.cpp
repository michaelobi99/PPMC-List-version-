...
int main() {
	...
	try {
		...
		auto input1 = stl::OpenInputBitFile(R"(..\PredictionByPartialMatching(PPMC)\testFile2.txt)");
		std::fstream output1(R"(..\PredictionByPartialMatching(PPMC)\testFile3.txt)", std::ios_base::out | std::ios_base::binary);
		...
		//print file sizes
		std::cout << std::format("Original file size = {} bytes\n", fileSize(fs::path(R"(..\PredictionByPartialMatching(PPMC)\testFile.txt)")));
